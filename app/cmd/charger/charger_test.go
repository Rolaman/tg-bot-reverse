...
func TestSendMessageWithEnoughBalance(t *testing.T) {
	...
	bot.Handle(tele.OnText, func(c tele.Context) error {
		assert.Equal(t, "321", c.Text())
		return nil
	})

	message := events.SQSMessage{Body: "1:123"}
	err = sqsHandler(bot, client, message)
	balance, err := client.Balance("1")
	if err != nil {
		t.Fatal(err)
	}
	assert.Lessf(t, balance, 100, "balance should be charged")
}
